...
# KERNEL_MAKE_ARGS=menuconfig make kernel
	...
	c=$PKGS_DIR/$PKG_NAME/configs/${NAMTSO_BOARD}.config
	d=$PKGS_DIR/$PKG_NAME/configs/${NAMTSO_BOARD}_defconfig
	D=arch/arm64/configs/${NAMTSO_BOARD}_defconfig
	...
	[ "$d" -a -s "$d" ] && {
		...
		cp "$d" "$D"
		KERNEL_MAKE_ARGS="${NAMTSO_BOARD}_defconfig savedefconfig $KERNEL_MAKE_ARGS"
	    }
	}
