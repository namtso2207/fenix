...
display_parameters() {
	...
	if [ "$CREATE_ROOTFS_CACHE_ONLY" != "yes" ]; then
		echo "Namtso Board:          $NAMTSO_BOARD"
		[ "$NAMTSO_BOARD" != "Generic" ] && echo "Uboot Version:         $UBOOT"
		[ "$NAMTSO_BOARD" != "Generic" ] && echo "Uboot Configuration:   $UBOOT_DEFCONFIG"
		[ "$NAMTSO_BOARD" != "Generic" ] && echo "Uboot Branch:          $UBOOT_GIT_BRANCH"
...
build_debs() {
	...
	if [ "$FORCE_BUILD_KERNEL_DEB" == "yes" ]; then
		...
		if [[ ! -f $BUILD_DEBS/$VERSION/$NAMTSO_BOARD/${LINUX_IMAGE_DEB}_${VERSION}_${DISTRIB_ARCH}.deb ]]; then
			build_linux_debs
		else
			# Debs exist, but kernel version changed
			LINUX_DEB_VER=$(dpkg --info $BUILD_DEBS/$VERSION/$NAMTSO_BOARD/${LINUX_IMAGE_DEB}_${VERSION}_${DISTRIB_ARCH}.deb | grep Descr | awk '{print $(NF)}') #' coloring bug
