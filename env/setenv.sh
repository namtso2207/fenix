...
CONFIG_ARGS="NAMTSO_BOARD LINUX UBOOT DISTRIBUTION DISTRIB_RELEASE DISTRIB_RELEASE_VERSION
...
EXAMPLES:
    ...
    source setenv.sh NAMTSO_BOARD=A10-3588                  # interractive mode for A10-3588
    source setenv.sh NAMTSO_BOARD=A10-3588 --default	# non interractive mode
    source setenv.sh NAMTSO_BOARD=A10-3588 -d && echo OK           # same for scripting usage
    source setenv.sh NAMTSO_BOARD=wrong_board -d || echo ERROR  #
    source setenv.sh NAMTSO_BOARD=A10-3588 -s || echo ERROR SOME_PARAM UNDEFINED
    source setenv.sh NAMTSO_BOARD=A10-3588 LINUX=wrong_version -d || echo ERROR WRONG PARAM VALUE
    source setenv.sh NAMTSO_BOARD=A10-3588 LINUX=mainline COMPRESS_IMAGE=yes -d # generate next
    source setenv.sh NAMTSO_BOARD=A10-3588 LINUX=mainline UBOOT=mainline \\
...
## Choose namtso board
function choose_namtso_board() {

	echo_ ""
	echo_ "Choose namtso board:"
	i=0

	NAMTSO_BOARD_ARRAY=()
	for board in $ROOT/config/boards/*.conf; do
		if [ $(basename $board) != "Generic.conf" ]; then
			NAMTSO_BOARD_ARRAY+=("$(basename $board | cut -d'.' -f1)")
		fi
	done
	# Append Generic configuration to last
	if [ -f $ROOT/config/boards/Generic.conf ]; then
		NAMTSO_BOARD_ARRAY+=("Generic")
	fi

	NAMTSO_BOARD_ARRAY_LEN=${#NAMTSO_BOARD_ARRAY[@]}

	while [[ $i -lt $NAMTSO_BOARD_ARRAY_LEN ]]
	do
		BOARD_DESC=$(grep -m1 \#= $ROOT/config/boards/${NAMTSO_BOARD_ARRAY[$i]}.conf)
		BOARD_DESC=${BOARD_DESC#* }
		printf_ "%2s) %-8s - %s\n" \
		     $((i+1)) "${NAMTSO_BOARD_ARRAY[$i]}" "$BOARD_DESC"
		[ "${NAMTSO_BOARD_ARRAY[$i]}" = "$NAMTSO_BOARD" ] && return 0
		let i++
	done

	[ "$NOASK" -o "$AUTOFILL" ] || NAMTSO_BOARD=
	[ "$NAMTSO_BOARD" ] && return 1
	...
	do
		...
		if [ -n "`echo $ANSWER | sed -n '/^[0-9][0-9]*$/p'`" ]; then
			if [ $ANSWER -le $NAMTSO_BOARD_ARRAY_LEN ] && [ $ANSWER -gt 0 ]; then
				index=$((${ANSWER}-1))
				NAMTSO_BOARD="${NAMTSO_BOARD_ARRAY[$index]}"
			else
				wrong_num
			fi
		else
...
file '$ROOT/config/boards/${NAMTSO_BOARD}.conf'? Please add it!"
		return 1
	fi
...
choose_namtso_board              || err NAMTSO_BOARD    || return 1
oky NAMTSO_BOARD

cd $ROOT                                                || return 1
source config/boards/${NAMTSO_BOARD}.conf || { cd $CDIR;   return 1
}; cd $CDIR
